    ...
    I2C: I2c<Error = E>, 
{
    ...
    pub async fn set_datetime(&mut self, datetime: &DateTime) -> Result<(), Error<E>> {
        ...
        self.i2c.write(DEVICE_ADDRESS, &payload).await.map_err(Error::I2C)
    }

    /// Set only the time, date remains unchanged.
    ///
    /// Will return an 'Error::InvalidInputData' if any of the parameters is out of range.
    pub async fn set_time(&mut self, datetime: &Time) -> Result<(), Error<E>> {
    ...
    pub async fn get_century_flag(&mut self) -> Result<u8, Error<E>> {
        let flag = self.is_register_bit_flag_high(Register::CENTURY_MONTHS, BitFlags::C).await?;
        if flag {
            Ok(1)
        } else {
            Ok(0)
    ...
    pub async fn set_century_flag(&mut self, century: u8) -> Result<(), Error<E>> {
        match century {
            0 => self.clear_register_bit_flag(Register::CENTURY_MONTHS, BitFlags::C).await,
            1 => self.set_register_bit_flag(Register::CENTURY_MONTHS, BitFlags::C).await,
